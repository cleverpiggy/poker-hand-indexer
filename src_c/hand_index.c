...
// ADDED void -> bool
bool enumerate_configurations_r(uint_fast32_t rounds, const uint8_t cards_per_round[],
    ...
    // ADDED void (*observe) -> bool (*observe)
    bool (*observe)(uint_fast32_t, uint_fast32_t[], void*), void * data) {
  if (suit == SUITS) {
    // ADDED if false
    if (!observe(round, configuration, data)) {
      return false;
    }

    if (round+1 < rounds) {
      // ADDED if false
      if (!enumerate_configurations_r(rounds, cards_per_round, round+1, cards_per_round[round+1], 0, equal, used, configuration, observe, data)) {
        return false;
      }
    }
  } else {
    ...
    for(uint_fast32_t i=min; i<=max; ++i) {
      ...
      // ADDED if false
      if (!enumerate_configurations_r(rounds, cards_per_round, round, remaining-i, suit+1, new_equal, used, configuration, observe, data)) {
        return false;
      }
...
// ADDED void -> bool
bool enumerate_configurations(uint_fast32_t rounds, const uint8_t cards_per_round[],
    // ADDED void (*observe) -> bool (*observe)
    bool (*observe)(uint_fast32_t, uint_fast32_t[], void*), void * data) {
  uint_fast32_t used[SUITS] = {0}, configuration[SUITS] = {0};
  return enumerate_configurations_r(rounds, cards_per_round, 0, cards_per_round[0], 0, (1<<SUITS) - 2, used, configuration, observe, data);
}
...
// ADDED void -> bool
bool tabulate_configurations(uint_fast32_t round, uint_fast32_t configuration[], void * data) {
...
out:;
  ...
  for(uint_fast32_t i=0; i<SUITS;) {
    ...
    // ADDED
    if (size+SUITS-1 >= MAX_GROUP_INDEX) {
      return false;
    }
...
bool hand_indexer_init(uint_fast32_t rounds, const uint8_t cards_per_round[], hand_indexer_t * indexer) {
  ...
  memset(indexer->configurations, 0, sizeof(indexer->configurations));
  // ADDED if false
  if (!enumerate_configurations(rounds, cards_per_round, tabulate_configurations, indexer)) {
    hand_indexer_free(indexer);
    return false;
  }
  ...
  for(uint_fast32_t i=0; i<rounds; ++i) {
    ...
    // ADDED
    // compiler says it will alway evaluate to true
